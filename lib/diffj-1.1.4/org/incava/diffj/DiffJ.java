...
import org.incava.io.*;
import org.incava.java.*;
import org.incava.qualog.Qualog;
import org.incava.util.Arrays;
...
public class DiffJ
{
    ...
    public DiffJ(String[] args, OutputStream stream)
    {
        ...
        if (opts.briefOutput) {
            report = new BriefReport(stream);
        }
        else {
            report = new DetailedReport(stream);
        }
    ...
    protected void process(File from, File to)
    {
        ...
        if ((fromIsStdin || from.isFile()) && (toIsStdin || to.isFile())) {
            ...
            try {
                ...
                // Added by Frank; hopefully prevents java's "too many open files"
                if(fromRdr != null)
                	fromRdr.close();	
                if(toRdr != null)
                	toRdr.close();
    ...
    {
        DiffJ dj = new DiffJ(args, System.out);
